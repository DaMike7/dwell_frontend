...
const PropertyOwnersSignup = () => {
    ...
    const handleUrlChange = (value) =>{
        let enteredUrl = value

        // Check if the entered URL doesn't alreAdamaway start with "https://"
        if (enteredUrl !== '' && !enteredUrl.startsWith('https://www.')) {
            enteredUrl = 'https://www.' + enteredUrl;
        }
    ...
    const handlePOSignup = async (e) => {
        e.preventDefault();

        try {
            const response = await client.post('users/property-owners/signup/', {
                username:username,
                email:email,
    ...
    const all_positions = [
        {"name":"Owner","psCode":"owner"},
        {"name":"Property Manager","psCode":"property_manager"},
    ]
    ...
    const handleStateChange = (value , setstate) => {
        ...
        // Fetch and set the corresponding cities for the selected state
            ...
            <div className="">
                <div className="">
                    ...
                    <div class="flex flex-col items-center justify-center px-6 py-8 ">
                        ...
                        <div class="w-full bg-white rounded-lg shAdamawaow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                                <h1 class="text-xl font-bold leAdamawaing-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                                ...
                                <form class="space-y-4 md:space-y-6">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <select required onChange={(e) => handlePositionChange(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" id="selectPosition">
                                            <option value=""></option>
                                            {all_positions.map(positions => (
                                            <option value={positions.psCode}>{positions.name}</option>
                                            ))}
                                        </select>
                                    </div>
                                    {/*WEBSITE URL */}
                                    <div>
                                        <label for="websute" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Website Url (optional)</label>
                                        <input type="text" id="website_url" value={url} onChange={(e) =>handleUrlChange(e.target.value)} class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="www.mywebsite.com" required/>
                                    </div>
                                    ...
                                    {/*POST BUTTON */}
                                    <button onClick={handlePOSignup} class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Create an account</button>
                                    {/* Error message **/}
                                    {error && (
...
export default PropertyOwnersSignup;